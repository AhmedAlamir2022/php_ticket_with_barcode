...
<head>
	...
	<!-- PAGE TITLE HERE ============================================= -->
	<title>Black Market || My Tickets </title>
...
{ ?>
								...
								<div class="profile-tabnav">
									<ul class="nav nav-tabs">
										...
										<li class="nav-item">
											<a class="nav-link active" href="my-tickets.php"><i class="ti-bookmark-alt"></i>My Tickets </a>
										</li>
										<li class="nav-item">
						...
						<div class="col-lg-12 col-md-12 col-sm-12">
							<div class="row">
								<?php 
                  $user_email=$_SESSION['login'];
                  $sql = "SELECT ttypes.ttype,tickets.*  from tickets join ttypes on ttypes.type_id=tickets.ticket_type where user_email=:user_email";
                  $query = $dbh -> prepare($sql);
                  $query -> bindParam(':user_email',$user_email, PDO::PARAM_STR);
                  $query->execute();
                  $results=$query->fetchAll(PDO::FETCH_OBJ);
                  ...
                  { ?>
                            ...
                            if( $result->checkout ==1){ ?>
                             <a href="ticket-info.php?id=<?php echo htmlentities($result->ticket_id);?>" target="_blank" ><button class="btn btn-success">Get  info</button></a>
                            
                              <?php }
                            else{ ?>
                            <a href="checkout.php?id=<?php echo htmlentities($result->ticket_id);?>"><button class="btn btn-success">Checkout</button></a>
                            <?php } ?></h5>
											</div>
